...
- `derived_temp_queue_name_with_uid` and
  `derived_temp_queue_name_with_uid_and_identifier` for plugin managers and
  inceptors that need derived queue names scoped by namespace, BGD name, BGD
  UID, inception point, role, logical purpose, and optional semantic queue
  identifier token. The operator treats those names as opaque plugin config.
