...
- `supportedRoles`
- `topology`
- `inceptor`, including image, ports, env, volume mounts, optional volumes,
...
- Inceptor lifecycle endpoints do not verify signatures and do not know the
  ...
  `FLUIDBG_PLUGIN_AUTH_TOKEN`.
- Application or verifier calls to inceptor-owned APIs, such as HTTP `/write`,
  must use the same `Authorization: Bearer <token>` value for that inception
  point.
...
- Inceptor to verifier `observer.notifyPath` calls set
  `Authorization: Bearer <token>`, so the verifier can reject callbacks from
  unrelated pods.
- HTTP `mock` calls to the verifier set the same bearer token and return the
  verifier response to the original application caller.
- Operator polling of verifier `verifyPath` uses the token that was presented
  during `/testcases` registration. The verifier can therefore reject result
  polling for cases that were not registered by a valid inceptor.
...
JWT authentication and TLS are separate controls. JWTs prove the caller identity;
TLS encrypts the hop and, when certificate verification is enabled, proves the
server identity.

All built-in control-plane HTTP channels default to plain HTTP for local
clusters and simple in-cluster installs. They can be upgraded independently:

| Channel | Direction | TLS configuration |
|---|---|---|
| Operator API | Inceptor to operator `/testcases`; operator to verifier `verifyPath` uses the verifier URL | `operator.api.tls.enabled=true` in Helm starts the operator API with HTTPS. Use `operator.api.tls.certPath` and `operator.api.tls.keyPath` for the server cert/key. |
| Operator to manager | Operator calls manager `prepare`, `cleanup`, and `sync` | `InceptionPlugin.spec.manager.controlPlaneTls.enabled=true` makes the operator call the manager Service with `https://service.namespace.svc`. |
| Operator to inceptor | Operator calls inceptor `prepare`, `activate`, `traffic`, `drain`, `drain-status`, and `cleanup` | `InceptionPlugin.spec.inceptor.controlPlaneTls.enabled=true` makes the operator call the inceptor Service with `https://service.namespace.svc`. |
| Inceptor server | Manager/inceptor process serving lifecycle APIs | Built-in plugins serve HTTPS when `FLUIDBG_CONTROL_PLANE_TLS_ENABLED=true` with cert/key paths. The HTTP plugin starts a separate HTTPS control-plane listener, so app proxy HTTP/HTTPS can remain configured independently. |

Publicly trusted certificates work without `caCertPath`; the Rust clients use
their normal TLS trust store. For private cluster CAs, mount the CA into the
caller pod and set `caCertPath`. `insecureSkipVerify` exists only for disposable
local tests and disables server certificate verification.

Only the signing Secret is long-lived. Per-inception JWTs are generated by the
operator and injected into inceptor pods through generated Secrets and
`secretKeyRef`, not literal env values. When a BGD has a verifier, the operator
also stores `FLUIDBG_VERIFIER_AUTH_TOKENS_JSON` in a generated verifier Secret
and injects it through `secretKeyRef`. The value is a JSON object mapping
inception point names to the token expected for that inception point. The
verifier gets tokens only; it never receives the signing key. The operator also
stores the token presented during test-case registration with that test-case
record so later verifier polling can use the same bearer token.
Temporary inceptor Deployments, Services, ConfigMaps, Pods, and per-inception
auth Secrets are deleted after promotion, rollback, or rollout restart cleanup.
The selected operator signing Secret is user-owned and is not deleted by rollout
cleanup.
