...
When a BGD has a verifier, the operator also injects
`FLUIDBG_VERIFIER_AUTH_TOKENS_JSON` into the verifier Deployment. The value is a
JSON object mapping inception point names to the token expected for that
inception point. The verifier gets tokens only; it never receives the signing
key.
