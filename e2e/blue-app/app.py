...
async def handle_core_nats_message(nc, msg):
    try:
        ...
        print(f"blue-app failed to process core NATS message: {exc}", flush=True)
...
async def core_nats_main():
    while True:
        try:
            ...
            mark_ready()
            print(
                f"blue-app consuming core NATS subject={INPUT_SUBJECT} queueGroup={NATS_QUEUE_GROUP}",
                flush=True,
            )
            while True:
                try:
            ...
            print(f"blue-app core NATS error: {exc}, reconnecting...", flush=True)
