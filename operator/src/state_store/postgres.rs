...
fn row_to_test(row: &sqlx::postgres::PgRow) -> InceptionTest {
    use sqlx::Row;
    ...
    InceptionTest {
        ...
        trigger_inception_point: row.get("trigger_inception_point"),
...
impl StateStore for PostgresStore {
    async fn register(&self, run: InceptionTest) -> Result<()> {
        ...
        let query = format!(
            r#"INSERT INTO {table} (test_id, blue_green_ref, triggered_at, trigger_inception_point, timeout_seconds, status, verdict, expires_at)
        ...
        sqlx::query(&query)
            ...
            .bind(&run.trigger_inception_point)
    ...
    async fn set_verdict(&self, test_id: &str, passed: bool) -> Result<()> {
        ...
        let query = format!(
            "UPDATE {} SET status = $1, verdict = $2 WHERE test_id = $3 AND status IN ('Triggered', 'Observing')",
            self.table_name
        );
